...
<body>
    ...
    <div class="form">
        <h2>Update <?php echo e($employee->name); ?></h2>
        <br>
        <form action="/update-user/<?php echo e($employee->id); ?>" method="POST" enctype="multipart/form-data">
        <?php echo csrf_field(); ?>
        <label for="name" class="add1">Name :
            <input class="form-control" type="text" name="name" id="name" value="<?php echo e($employee->name); ?>">
        </label>
        <br>
        ...
        <label for="email" class="add1">Email :
            <input class="form-control" type="text" name="email" id="email" value="<?php echo e($employee->email); ?>">
        </label>
        <br>
        <label for="division" class="add1">
            ...
            <select name="division" id="division" class="form-select">
                <option></option>
                <option value="Jakarta" class="form-select" <?php
                    if ($employee->division == 'Jakarta') {
                        echo 'selected' ;
                    }
                ?>>Jakarta</option>
                <option value="Bekasi" class="form-select" <?php
                if ($employee->division == 'Bekasi' ) {
                    echo 'selected' ;
                }
                ?>>Bekasi</option>
            </select>
        ...
        <label for="role" class="add1">
            ...
            <select name="role" id="role" class="form-select">
                <option></option>
                <option value="1" class="form-select" <?php
                    if ($employee->role == 'employee') {
                        echo 'selected' ;
                    }
                ?>>Employee</option>
                <option value="2" class="form-select" <?php
        ...
        <br>
        <label for="image" class="add1">Change Image :
            <input class="form-control" type="file" name="image" id="image" value="<?php echo e($employee->image); ?>">
        </label>
        <br>
        <p class="add1">Current Image : </p>
        <img class="picture" src="<?php echo e(asset('images/'.$employee->image)); ?>" alt="" style="width: 80px">
        <span class="picture-text" name="old" value="<?php echo e($employee->image); ?>" hidden><?php echo e($employee->image); ?></span>
        <input type="hidden" value="<?php echo e($employee->image); ?>" name="old">
        <p class="add1">New Image :</p>
        <img class="preview" id="blah" src="#" alt="Image Preview"/>
        <br><br>
        <input type="hidden" value="<?php echo e($employee->password); ?>" name="oldpass">
        <div class="duo-button">
            <button class="batal" style="color: #fff;">Cancel</button>
            <button class="submit"type="submit" name="sumbit" style="color: #fff;">Update</button>
        </div>
        </form>
    </div>
    ...
    <script>
        ...
        cancelButton.addEventListener('click', function(event) {
            ...
            if (confirmCancel) {
                // Redirect the user to the /manage-employee page or any other desired action
...
<?php /**PATH /Users/lintang/Documents/laravel/bookingmeetingroomV2/resources/views/admin/employees/updateEmployees.blade.php ENDPATH**/ ?>