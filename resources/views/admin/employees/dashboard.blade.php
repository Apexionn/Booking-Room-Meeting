...
{{-- Search --}}
    <div class="container-search autocomplete-container">
        <form class="search-form" role="search" action="/manage-employees">
...
{{-- <script type="text/javascript">
    var route = "{{ route('autocomplete-users') }}";
    $('#search').typeahead({
        ...
        <tbody>
            ...
            <tr class="table-row {{ $no % 2 == 0 ? 'even-row' : 'odd-row' }}">
                    ...
                    <td>
                        @php
                        @endphp
                        <img class="fotopfp" src="{{ asset('images/'.$user->image) }}" alt="" style="width: 80px">
                    </td>
                    <td scope="row">
                        <div class="tes">
                                <a href="/select-user/{{ $user->id }}" class="edit">Edit</a>
                                <a href="/delete-user/{{ $user->id }}" class="delete">Delete</a>
                        </div>
                    </td>
                </tr>
            @endforeach
