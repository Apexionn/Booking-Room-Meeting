...
<body>
    <div class="logo">
        <a href="/manage-employees"><img src="{{ asset('IMG/logo.png') }}"></a>
    </div>
    <ul>
        <li>
            <div class="popover">
    ...
    <div class="form">
        ...
        <br>
        <form action="/update-user/{{ $employee->id }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <label for="image" class="add1">Change Image :
            <input class="form-control" type="file" name="image" id="image" value="{{$employee->image }}">
        </label>
        <br>
        <p class="add1">Current Image : </p>
        <img class="picture" src="{{ asset('images/'.$employee->image) }}" alt="" style="width: 80px">
        <span class="picture-text" name="old" value="{{ $employee->image }}" hidden>{{ $employee->image }}</span>
        <input type="hidden" value="{{$employee->image}}" name="old">
        <p class="add1">New Image :</p>
        <img class="preview" id="blah" src="#" alt="Image Preview"/>
        <br><br>
        <input type="hidden" value="{{$employee->password}}" name="oldpass">
