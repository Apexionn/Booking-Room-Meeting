...
<body>
    ...
    <div class="form">
        ...
        <h2 class="heading2">{{ $roomData->name }}</h2>
        <br>
        <form action="/update-room/{{ $id }}" method="POST" enctype="multipart/form-data" id="roomForm">
            @csrf
        <label for="name" class="add1">Name :
            <input class="form-control" type="text" name="name" id="name" value="{{$roomData->name }}">
        </label>
        <br>
        <label for="image" class="add1">Change Image :
            <input class="form-control" type="file" name="image" id="image" value="{{$roomData->image }}">
        </label>
        <br>
        <p class="add1">Current Image : </p>
        <img class="picture" src="{{ asset('images/'.$roomData->image) }}" alt="" style="width: 80px">
        <span class="picture-text" name="old" value="{{ $roomData->image }}" hidden>{{ $roomData->image }}</span>
        <input type="hidden" value="{{$roomData->image}}" name="old">
        <br> <br>
